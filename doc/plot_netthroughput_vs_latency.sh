...
for i, ws in enumerate(wsizes):
    ldata = data[(data[:,0] == ws) & (data[:,2] == $PACKETSIZE)]
    #ldata = data[ldata[:,2] == $PACKETSIZE] # max packet size
    ...
    plot_eplot          = sax.errorbar(ldata[:,1], ldata[:,3], yerr=ldata[:,4], fmt='.', label='\$\\mathrm{T}\$')
...
ax.set_ylabel(r"T: Throughput [\si[per-mode=symbol]{\mebi\byte\per\second}]", labelpad=35)
ax.set_xlabel(r"ACK Delay [\si{\micro\second}]", labelpad=35)
ax.set_xticks([])
ax.set_yticks([])
